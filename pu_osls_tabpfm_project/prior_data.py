...
def _sample_rows_features(rng: np.random.Generator, cfg: PriorGeneratorConfig) -> Tuple[int, int]:
    R = int(rng.integers(cfg.min_rows, cfg.max_rows + 1))
    F = int(rng.integers(cfg.min_features, cfg.max_features + 1))
    return R, F


def _sample_split_index(rng: np.random.Generator, R: int, cfg: PriorGeneratorConfig) -> int:
